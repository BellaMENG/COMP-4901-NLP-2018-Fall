...
def predict_final_word_models(models, vocabulary, filename, votes):
    id_list = []
    prev_tokens_list = []
    prev_tokens_lens = []
    with open(filename, "r") as fin:
    ...
    ind = 0
    for model in models:
        y_prob = y_prob + votes[ind]*model.predict(X, batch_size=32)
        ind = ind + 1
...
def main(opt):
    ...
    if opt.mode == "train":
        ...
        model3 = load_model(opt.saved_model3)
        model3.name = 'model3'
        for layer in model3.layers:
            layer.name = layer.name + str("_3")
        
        model4 = load_model(opt.saved_model4)
        model4.name = 'model4'
        for layer in model4.layers:
            layer.name = layer.name + str("-4")
        models = [model1, model2, model3, model4]
        
        vocabulary = json.load(open(os.path.join("data", "vocab.json")))
        votes = [opt.vote1, opt.vote2, opt.vote3, opt.vote4]
        
        predict_dict = predict_final_word_models(models, vocabulary, opt.input, votes)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-saved_model1", type=str, default="model.h5",
                        help="saved model path")
    parser.add_argument("-saved_model2", type=str, default="model.h5",
                        help="saved model path")
    parser.add_argument("-saved_model3", type=str, default="model.h5",
                        help="saved model path")
    parser.add_argument("-saved_model4", type=str, default="model.h5",
                        help="saved model path")
    parser.add_argument("-model_to_be_saved", type=str, default="model.h5",
                        help="ensembled model")

    parser.add_argument("-vote1", type=float, default=0.5,
                        help="voting right for model1")
    parser.add_argument("-vote2", type=float, default=0.5,
                        help="voting right for model2")
    parser.add_argument("-vote3", type=float, default=0.0,
                        help="voting right for model3")
    parser.add_argument("-vote4", type=float, default=0.0,
                        help="voting right of model4")
